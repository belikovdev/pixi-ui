...
class ScrollBox extends pixi_js.Container {
  ...
  constructor(options) {
    ...
    __publicField(this, "proximityRange", 0);
  ...
  makeScrollable() {
    ...
    this.on("pointerdown", (e) => {
      ...
      if (this._trackpad && this.dragStarTouchPoint) {
        this._trackpad.pointerDown(new pixi_js.Point(this.dragStarTouchPoint.x, this.dragStarTouchPoint.y));
      }
    ...
    this.on("globalpointermove", (e) => {
      ...
      this._trackpad?.pointerMove(new pixi_js.Point(touchPoint.x, touchPoint.y));
  ...
  resize(force = false) {
    ...
    if (this._trackpad) {
      const maxWidth = this._width - (this.list?.width ?? 0) - (this.list?.leftPadding ?? 0) - (this.list?.rightPadding ?? 0);
      const maxHeight = this._height - (this.list?.height ?? 0) - (this.list?.topPadding ?? 0) - (this.list?.bottomPadding ?? 0);
      if (this.isBidirectional) {
